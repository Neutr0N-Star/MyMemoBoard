...
- 修复「检查更新」卡死无反馈：检测到新版后改为弹出浏览器打开 GitHub Releases 下载页，由浏览器接管下载，不再走 App 内后台下载（原方式在部分网络下会无声挂起）

## 下载

| 平台 | 文件 | 说明 |
|---|---|---|
| Windows | `MyMemoBoards-1.0.3-setup.exe` | NSIS 安装版（开始菜单/桌面快捷方式名：知墙）——**正式发布仅提供此安装包** |

> 另有 `MyMemoBoards-1.0.3-portable.exe` 单文件便携版（数据在 exe 同目录 `data/`），仅作者内测自用，**不随正式发布提供**。
