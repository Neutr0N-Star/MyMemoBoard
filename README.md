...
一款单文件个人事务工作台，核心是**可自定义增删的便签墙**。**定稿视觉 = 构成主义·工程设计流**（蓝图网格 / 机械剖线 / 红黑白功能色）。

## 快速开始

**桌面应用**：双击 `release\MyMemoBoards-1.0.0-setup.exe` 安装 → 独立窗口，无浏览器地址栏/标签页，视觉完整，自带构成主义图标。另有便携版 `-portable.exe` 供内测自用（不随正式发布提供）。
...
| 便签墙（可自定义） | 增、删、改名、拖拽排序、设默认墙；空态启动，不预置示例 |
| 便签查看/修改 | 点击墙上的便签，浮窗里可查看/修改标题、墙、截止、时间、课程、重要程度、每日任务 |
| 课程清单 | 课程筛选芯片（点哪门只看哪门）+ 课程便签 |
...
**首次打开（发布版空态）**：不再预置任何示例数据——便签、作息、课程、考试全是空的，各区域显示引导占位（「点 + 贴第一张便签」「点 + 添加考试」等），像正式产品第一次启动。所有弹窗提示都改成自绘确认框 / toast，不依赖浏览器原生 `confirm`/`alert`。如果之前打开过旧版本、预览里还留着旧数据，去「设置 → 重置全部数据」一键清空回首次打开状态。
...
**AI 周报**：只填 Base URL / Key / 模型名，走 OpenAI 兼容的 `/v1/chat/completions`。本地跑了 Ollama 就填 `http://localhost:11435/v1`，不花钱。生成失败时会把本来要发的原始数据摊开给你，方便排查（常见是端口不对——用桌面 exe 更稳）。桌面版会在保存 Key 时用系统 `safeStorage` 加密存储，不外发。

**桌面 exe 专属**：顶栏「置顶」按钮 + 系统通知（上课前 5 分钟、今日待办提前 30 分钟，设置里可关），设置里还可开启「开机启动」「自动备份」「检查更新」。这些能力经 `preload.js` 用 `contextBridge` 桥接，页面仍拿不到任何 node 能力。**改动后需重新打包才生效。**

**未签名提示**：当前 exe 未做代码签名，Windows SmartScreen 可能提示「未知发布者」。这是本地自用的正常现象；如需正式对外分发，建议后续配置代码签名证书。

**下载与校验**：发布时建议同时提供 `SHA256SUMS.txt`。用户可运行 `Get-FileHash -Algorithm SHA256 .\MyMemoBoards-1.0.0-setup.exe` 与 `release/SHA256SUMS.txt` 对比，防止文件被篡改。完整发布说明模板见 `docs/RELEASE.md`，GitHub Pages 下载页见 `docs/index.html`。
...
- **默认全离线**：一切便签、作息、课程、设置都只存在本机——桌面版在 exe 同目录 `data/`（不写 C 盘 AppData），不上传任何服务器。
- **AI 周报**：只有你主动点「生成」时，才会把本周便签/完成/逾期/考试等摘要发往你**自己填的** OpenAI 兼容 Base URL（如本地 Ollama）。默认 Base URL 为空，不会请求网络。
- **API Key**：你填的 Key 不会发给作者或任何第三方；桌面版会用系统 `safeStorage` 加密后保存在本地数据中；**导出 JSON 时 Key 会被替换为 `***`**，导入脱敏文件不会覆盖你当前已保存的 Key。
...
- 安装版（发布渠道）：`release\MyMemoBoards-1.0.0-setup.exe`（NSIS 安装，开始菜单/快捷方式/卸载）——对外正式发布只提供此安装包。
- 便携版（内测自用）：`release\MyMemoBoards-1.0.0-portable.exe`（单文件 portable，可重命名为任意文件名；双击即开，数据落在 exe 同目录 `data\`）——仅作者内测用，不随发布提供。
- 命令行开发：`npm start`
...
npm run dist           # 产出 portable + setup（release/MyMemoBoards-1.0.0-portable.exe 与 -setup.exe）
...
注意：本项目主进程（`main.js`）目前**不会**自动清除该变量；如遇同样报错，手动 `set ELECTRON_RUN_AS_NODE=` 清掉后再启动即可。

## 入口

- 门牌：`.overview.md`；蓝图：`notes.md`；产物：`index.html` + `assets/archive/`。
- 桌面外壳：`main.js` + `preload.js` + `package.json` + `assets/icon.ico`；打包产物 `release/MyMemoBoards-1.0.0-portable.exe` 与 `release/MyMemoBoards-1.0.0-setup.exe`。
