...
| 模块 | 说明 |
|---|---|
| 晷刻工作室 | 左上角方形徽章 logo（裁掉外框留白，直角无圆角），标题不再带「星之间」 |
| 墙面管理 | 四大墙可自行增减：点虚线框「+ 加大框」加墙、墙头 `×` 删墙（便签自动挪走不丢）、双击墙名改名 |
...
**墙面（大框）管理**：墙的数量和名字完全自己说了算。墙区末尾的虚线框「+ 加大框」点一下直接输入墙名回车，就多一面墙（比如「论文」「健身」）；每面墙的标题行右侧有个 `×`，点它删墙——确认后墙会消失，墙上的便签自动挪到第一面非科目墙，一张不丢（删「学期科目」墙还会清掉课程筛选）；双击墙名可以改名。删光所有墙会兜底留一面。移动端底部 Tab 仍是固定几个，自定义墙在「墙面」全览里看。
