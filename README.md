...
| 模块 | 说明 |
|---|---|
| 晷刻工作室 | 左上角方形徽章 logo：无边框、直接放大压线交叉顶栏格子（约便签盒纸堆宽度），直角无圆角 |
| 墙面管理 | 大框可增删改排：虚线框「+ 加大框」加墙、`×` 两次确认删墙（便签自动挪到默认墙）、双击墙名改名、`≡` 拖拽换墙位置、`D` 设默认墙 |
...
- **默认墙**：墙头 `D` 按钮，墨底反白的那面就是默认墙（删别的大框时便签挪到它）；删掉默认墙会自动换到第一面非科目墙。移动端底部 Tab 仍是固定几个，自定义墙在「墙面」全览里看。

**首次打开（发布版空态）**：不再预置任何示例数据——便签、作息、课程、考试全是空的，各区域显示引导占位（「点 + 贴第一张便签」「点 + 添加考试」等），像正式产品第一次启动。所有弹窗提示都改成自绘确认框 / toast，不依赖浏览器原生 `confirm`/`alert`（iframe 预览里原生弹窗会被沙箱静默拦掉，点了没反应）。
